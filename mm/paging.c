...
{
  struct page_table* pml4t = (void*) pmmgr_alloc_page();
  ...
  for (u64_t gb = 0; gb < 4; gb++) {
    struct page_table* pd = (void*) pmmgr_alloc_page();
                      ...
                      struct page_entry_flags  flags)
{
  ...
  struct page_table* pml4t = (struct page_table*) GET_PML4T_VADDR();
  ...
  struct page_entry *pdpte = &pdpt->entries[pdpt_idx];
  if (!pdpte->present) {
  ...
  struct page_entry *pde = &pd->entries[pd_idx];
  if (!pde->present) {
  ...
  struct page_entry *pte = &pt->entries[pt_idx];
