...
{
  u64_t cap_reg = *(u64_t*)(hpet_base_reg + HPET_GENERAL_CAPABILITIES_REGISTER_OFFSET);
  return (u32_t)(cap_reg >> 32);
}

void hpet_enable(void* hpet_base_reg, bool legacy_replacement)
{
  u64_t* reg = (u64_t*)(hpet_base_reg + HPET_GENERAL_CONFIG_REGISTER_OFFSET);
  *reg &= ~(0b11);   // clear
...
{
  return *(u64_t*)(hpet_base_reg + HPET_MAIN_COUNTER_REGISTER_OFFSET);
}
